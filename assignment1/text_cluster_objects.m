...
for r = 1:size(bag_text_vector, 2)
  text_vector(r,:) = bag_text_vector(r).vector;
  filenames{r} = bag_text_vector(r).name; 
end

%% For Shoes.
start = size(text_vector,1);
for r = 1:size(shoes_text_vector, 2)
  text_vector(r+start,:) = shoes_text_vector(r).vector;
  filenames{r+start} = shoes_text_vector(r).name; 
end
