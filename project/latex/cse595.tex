...
\maketitle
\begin{abstract}
We present a method to suggest tags based on the existing tags provided by
...
the suggested tags to the image under consideration.
\end{abstract}

\section{Introduction}
Flickr provides an image sharing service for users, and allows them to tag them
with descriptive words in order to help users find these images using keywords and
search terms. In some cases, the tags asssigned to the image are not exhaustive -
there remain objects in the potograph in the image that are not described in the
tags. Conversely, some tags are not descriptive, sometimes the indicate the
context/circumstance around the photograph - for example, the camera model, the date
etc.

We wish to alleviate both the problem of eliminating noisy tags from a photograph, and
also to bring in additional relevant tags, so that users are able to search and
query for images.

\section{Previous work}
At least two teams have tried to tackle this problem before us. The first approach,
proposed by Sevil et. al., proposed additional tags by querying a set of similar
images (using the original tags), and weighting the tags contained within them by the
similarity of the image to the original image. The second team, from Microsoft research,
used a randolm walk approach to suggest additional tags.
